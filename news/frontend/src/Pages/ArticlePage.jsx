...
const ArticlePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetch(`${API_ENDPOINT}/api/articles/?page=${currentPage + 1}`);
                      ...
                      <div>
                        ...
                        <div className='hidden sm:block mt-8'>
                          ...
                          {data    //filter archived false
                           //.filter(article => !article.archived)
                           .map((article, index) => (
                          <div key={article.id} className={`flex-row border-b-2 pb-4 mx-1 content-center border-[#795C34] my-10 ${
                            index === data.length - 1 ? 'border-b-0' : ''
                          }`}> 
                                <Link to={`/articles/${article.id}`}>
                                    <div>
